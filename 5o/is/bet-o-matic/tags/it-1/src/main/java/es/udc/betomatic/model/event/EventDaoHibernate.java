...
public class EventDaoHibernate
	...
	private String getSubstringsQuery(List<String> substrings){
		...
		if ((substrings!=null) && (substrings.size()>0)) {
			for (int i=0; i<substrings.size(); i++){
				str = str + "LOCATE(UPPER('"+substrings.get(i)+"'),UPPER(o.name))<>0 AND ";
			}
		}
		return str;
	}
	@SuppressWarnings("unchecked")
	public List<Event> findEvents(List<String> substrings, Calendar startDate,
	...
	@SuppressWarnings("unchecked")
	public List<Event> findEvents(List<String> substrings, long catId, Calendar startDate,
			int startIndex, int count ){
		try{
			String str = getSubstringsQuery(substrings);
            return getSession().createQuery(
