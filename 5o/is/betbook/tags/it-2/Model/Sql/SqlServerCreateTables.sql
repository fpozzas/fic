...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('[Countries]')
AND type in ('U')) DROP TABLE [Countries]
GO

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('[Languages]')
AND type in ('U')) DROP TABLE [Languages]
GO
...
CREATE TABLE Countries (
    ...
    CONSTRAINT PK_Countries PRIMARY KEY (countryCode, languageCode),
    CONSTRAINT FK_Languages FOREIGN KEY (languageCode) REFERENCES Languages (languageCode)
)
