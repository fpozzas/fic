...
let rec dev_pos lista pos = match lista with
	...
	| _ -> raise (Failure "Posición inválida")
...
let rec comm env = function
    ...
    | MkVar(name, value)  -> 
    	(match value with
    		VBool(b) -> Env.add env name (Bool (bexp env b))
    		| VInt(i) -> Env.add env name (Int (aexp env i))
    		| VString(s) -> Env.add env name (String (sexp env s)))

    | MkArray(name, size, value)  -> (if (aexp env size)<1 then raise (Failure "Número de posiciones incorrecto") 
		else let crear_lista valor tamano = 
			let rec cl_aux l v t =
				if t>0 then cl_aux ((t-1,v)::l) v (t-1) else l
			in cl_aux [] valor tamano
		in match value with
    		VBool(b) -> Env.add env name (ArrayBool (crear_lista (bexp env b) (aexp env size)))
    		| VInt(i) -> Env.add env name (ArrayInt (crear_lista (aexp env i) (aexp env size)))
    		| VString(s) -> Env.add env name (ArrayString (crear_lista (sexp env s) (aexp env size))))
		...
		(if not (exists env name) then raise (Tipos.NO_LIGADA name)
				...
				| VString(s) -> match s with 
					...
					| StringArray(n,x) -> (match (get env n) with
										...
										| _ -> raise (Tipos.ERROR_DE_TIPOS("")))
					| _ -> Env.add env name (String (sexp env s)))
