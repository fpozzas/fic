...
// Variables de los manejadores
int man_contador[32]; // Contador de ejecuciones, de 0 a maximo
int man_sleep[32]; // Tiempo de espera, de 0 a maximo
int man_verbose[32]; // Imprimir info, toma 0 o 1
int man_reenvio[32]; // Reenviar senal al proceso, toma 0 o 1
int maxreenvios = 0; // Numero de reenvios, 0 denota infinitos
int reenviosaux = -1; // Contador auxiliar para reenviar las senales
...
int procesar(char ** trozos){
	...
	// Comandos de p5
	else if (strcmp(trozos[0],"pila")==0) comm_pila(trozos);
	else if (strcmp(trozos[0],"senal")==0) comm_senal(trozos);
	else if (strcmp(trozos[0],"mask")==0) comm_mask(trozos);
	else if (strcmp(trozos[0],"ignore")==0) comm_ignore(trozos);
	else if (strcmp(trozos[0],"default")==0) comm_default(trozos);
	else if (strcmp(trozos[0],"handler")==0) comm_handler(trozos);
	else if (strcmp(trozos[0],"cont")==0) comm_cont(trozos);
	else if (strcmp(trozos[0],"bucle")==0) comm_bucle();
	else if (strcmp(trozos[0],"segmentation")==0) comm_segmentation();
	else if (strcmp(trozos[0],"fpe")==0) comm_fpe();
	else if (strcmp(trozos[0],"maxhandler")==0) comm_maxhandler(trozos);
...
int comm_pila(char ** trozos){
	int tam;
	void * dir;
	stack_t pila;
	if (trozos[1]==NULL){
		if (sigaltstack(NULL,&pila)<0) {perror("Error sigaltstack"); return -1;}
		printf("Direccion: %p\nTamano: %i\n",pila.ss_sp,pila.ss_size);
		return 0;
	}
	tam = strtol(trozos[1],(char **)NULL,10);
	if (trozos[2]==NULL){
		dir = malloc(tam);
		if (dir==NULL) {printf("Error malloc: no se ha podido reservar memoria"); return -1;}
	}
	else dir = (void *) strtol(trozos[2],(char **)NULL,0);
	pila.ss_sp = dir;
	pila.ss_flags = 0;
	pila.ss_size = tam;
	if (sigaltstack(&pila,NULL)<0) {perror("Error sigaltstack"); return -1;}
}
...
int print_signal_info(int signal){
	struct sigaction act;
	sigset_t ss1,ssproc;
	if (sigaction(signal,NULL,&act)<0) {perror("Error sigaction"); return -1;}
	// Nombre
	printf("%s   \t",NombreSenal(signal));
	// Enmascarada?
	sigprocmask(0,NULL,&ssproc);
	if (sigismember(&ssproc,signal)) printf("Enmascarada  \t");
	else printf("No enmascarada\t");
	// Manejador?
	if (act.sa_handler == SIG_IGN) printf("Ignorada    \t");
	else if (act.sa_handler == NULL) printf("Por defecto   \t");
	else printf("Manejador en %p\t",act.sa_handler);
	// Flags?
	if ((SA_NOCLDSTOP & act.sa_flags) == SA_NOCLDSTOP) printf("SA_NOCLDSTOP ");
	if ((SA_NOCLDWAIT & act.sa_flags) == SA_NOCLDWAIT) printf("SA_NOCLDWAIT ");
	if ((SA_RESETHAND & act.sa_flags) == SA_RESETHAND) printf("SA_RESETHAND ");
	if ((SA_ONSTACK & act.sa_flags) == SA_ONSTACK) printf("SA_ONSTACK ");
	if ((SA_RESTART & act.sa_flags) == SA_RESTART) printf("SA_RESTART ");
	if ((SA_NODEFER & act.sa_flags) == SA_NODEFER) printf("SA_NODEFER ");
	if ((SA_SIGINFO & act.sa_flags) == SA_SIGINFO) printf("SA_SIGINFO ");
	printf("\t");
	// Mascara asociada?
	int i;
	int flagprint = 1;
	for (i=1;i<MAXSIGNALS;i++){
		if (sigismember(&act.sa_mask,i)){
			if (flagprint) {printf("Mascara asociada "); flagprint=0;}
...
int comm_senal(char ** trozos){
	int sig;
	if (trozos[1]==NULL) print_all_signal_info();
	else while (trozos[1]!=NULL){
		sig = Senal(trozos[1]);
		if (sig>0) print_signal_info(sig);
		trozos++;
	}
}

int comm_mask(char ** trozos){
	int flag;
	sigset_t ss;
	if (trozos[1]==NULL) {print_all_signal_info(); return 0;}
	else if (!strcmp(trozos[1],"-n")) {flag = SIG_UNBLOCK; trozos++;}
	else flag = SIG_BLOCK;
	if (trozos[1]==NULL) {print_all_signal_info(); return 0;}
	else {
		sigemptyset(&ss);
		while(trozos[1]!=NULL){
			sigaddset(&ss,Senal(trozos[1]));
			trozos++;
		}
		sigprocmask(flag,&ss,NULL);
	}
	return 0;
}

int comm_ignore(char ** trozos){
	if (trozos[1]==NULL) print_all_signal_info();
	else while (trozos[1]!=NULL){
		sigignore(Senal(trozos[1]));
		trozos++;
	}
}

int comm_default(char ** trozos){
	if (trozos[1]==NULL) print_all_signal_info();
	else while (trozos[1]!=NULL){
		sigset(Senal(trozos[1]),SIG_DFL);
		trozos++;
	}
}

int comm_cont(char ** trozos){
	int i=1;
	int signal;
	char * nsenal;
	int borrar = 0;
	if ((trozos[1]!=NULL) && (!strcmp(trozos[1],"-r"))) {borrar = 1; trozos++;}
	if (trozos[1]==NULL) 
		while (i<MAXSIGNALS){
			if (borrar) man_contador[i]=0;
			else {
				nsenal = NombreSenal(i);
				if (strcmp(nsenal,"SIGUNKNOWN")) printf("%s -> %i\n",nsenal,man_contador[i]);
			}
			i++;
		}
	else 
		while (trozos[1]!=NULL){
			signal = Senal(trozos[1]);
			if ((signal>0) && (signal<MAXSIGNALS)){
				if (borrar) man_contador[signal] = 0;
				else printf("%s -> %i\n",NombreSenal(signal),man_contador[signal]);
			}
			trozos++;
		}
}
...
void manejador(int signal){
	man_contador[signal]++;
	if (man_verbose[signal]) 
		printf("Manejador de la senal %s, ejecucion %i, direccion del parametro %p\n",NombreSenal(signal),man_contador[signal],&signal);
	if (man_reenvio[signal]){
		if (maxreenvios==0) kill(getpid(),signal);
		else if (reenviosaux==-1) {reenviosaux = maxreenvios-1; kill(getpid(),signal);}
		else if (reenviosaux>0) {reenviosaux--; kill(getpid(),signal);}
		else reenviosaux = -1;
	}
	sleep(man_sleep[signal]);
}

int comm_handler(char ** trozos){
	int v = 0;
	int r = 0;
	int flags = 0;
	int d = 0;
	...
	while(trozos[1]!=NULL){
		if (!strcmp(trozos[1],"-v")) v = 1;
		else if (!strcmp(trozos[1],"-r")) r = 1;
		else if (!strcmp(trozos[1],"-p")) flags = flags | SA_ONSTACK;
		else if (!strcmp(trozos[1],"-t")) flags = flags | SA_RESETHAND;
		else if (!strcmp(trozos[1],"-f")) flags = flags | SA_NODEFER;
		else if (!strncmp(trozos[1],"-d",2)) d = strtol(trozos[1]+sizeof(char)*2, (char **)NULL, 10);
		else if (!strncmp(trozos[1],"-m",2)) sigaddset(&m,Senal(trozos[1]+sizeof(char)*2));
		else break;
		trozos++;
	}
	if (trozos[1]==NULL){printf("Parametros incorrectos\n"); return -1;}
	while (trozos[1]!=NULL){
		signal = Senal(trozos[1]);
		if ((signal>0) && (signal<MAXSIGNALS)){
			man_sleep[signal] = d;
			man_verbose[signal] = v;
			man_reenvio[signal] = r;
...
int comm_maxhandler(char ** trozos){
	if (trozos[1]==NULL) printf("maxreenvios == %i\n",maxreenvios);
	else maxreenvios = strtol(trozos[1],(char **)NULL,10);
	return 0;
}

int flag_bucle = 1;

void manejador_bucle(int signal){
	flag_bucle = 0;
}
...
int comm_bucle(){
	...
	sigemptyset(&ss);
	act1.sa_mask = ss;
	act1.sa_handler = manejador_bucle;
