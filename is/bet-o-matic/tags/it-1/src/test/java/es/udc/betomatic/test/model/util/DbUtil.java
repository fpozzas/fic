...
public class DbUtil {
	...
	public static void populateDb() throws Throwable {
		...
		Calendar firstJan = new GregorianCalendar();
		firstJan.set(2010, 0, 1);
		Category mus = categoryDao.find(new Long(3));
		Event testEvent = new Event("Jose Manuel - Antonio", firstJan, mus);
		/* Test event2: Cricket, Antonio's team - Otros, 01/01/2010 */
		Category cricket = categoryDao.find(new Long(1));
		Event testEvent2 = new Event("Antonio's team - Otros", firstJan, cricket);
		/* Test event3: Mus, Juan Jose - Pedro, 01/02/2010 */
		Calendar firstFeb = new GregorianCalendar();
		firstFeb.set(2010, 1, 1);
		Event testEvent3 = new Event("Juan Jose - Pedro", firstFeb, mus);
		...
		Set<BetOption> options = new HashSet<BetOption>(2);
